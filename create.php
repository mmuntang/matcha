...
session_start();
    ...
    if (isset($_POST['submit']))
    {
        ...
        if ($_POST['password']){
            $response = array('status' => false, 'statusMsg' => '<p class="warning">Password must have at least 6 characters, one digit and one capital letter.</p>');
...
try {
    ...
    header("Location: login.php?verify=ver");
    die(json_encode($response));
